...
//read a single sector of a drive. lba addresses only. driveNum can only be 0 or 1
char* ata_lba_read(unsigned int address, unsigned int driveNum = 0)
{
    ...
    char *hdd_read = (char*)malloc(sizeof(char) * 1024);
    for (int i = 0; i < 256; i++)
    {
        ...
        for (int i = 0; i < blocks->getSize(); i++)
        {
            ...
            *(unsigned int*)poop = blocks->at(i);
            ...
            //if (blocks->at(i) < 0xFFFFFF7)
            {
                ...
                {
                    unsigned int lbaOfFileCluster = ((blocks->at(i)-2) * sys->sectorsPerCluster) + rootLBA(sys) + s;
                    ...
                    char *bytes = ata_lba_read(lbaOfFileCluster, driveNum);
                    /*for (int u = 0; u < 512; u++)
                    {
                    ...
                    memcpy((fileStore+(((i*sys->sectorsPerCluster)+s) * 512)), (bytes), 512);        //memcpy is the most optimized memory copy right now
                    debugInt++;
